// =============================================================================
// step08 / step08.ts — 主循环 + 注册新的 agent-team 工具族
// -----------------------------------------------------------------------------
// 主循环结构与 step07 完全一致：skills 注入、上下文压缩、并发 tool calls。
// step08 的真正改动是工具集 + system prompt：
//
//   * 多注册了 spawn_agent / send_input / wait_agent / close_agent / list_agents，
//     让模型可以自己编排一支后台 agent 团队；
//   * spawn_sub_agent 仍然存在，但底层换成了“起后台 agent + 同步等结果”的实现。
//   * system prompt 多了一段说明，告诉模型这些新工具适合什么场景。
// =============================================================================
...
async function main(): Promise<void> {
  ...
  // System prompt 与 Rust step08.rs 保持一致：除了原来的 spawn_sub_agent 外，
  // 还要告诉模型“长任务优先用 spawn_agent + send_input + wait_agent + close_agent
  // + list_agents”这一套。
