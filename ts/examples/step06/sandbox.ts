// =============================================================================
// step06 / sandbox.ts — 在 step05 基础上加入并发工具调度
// -----------------------------------------------------------------------------
// 这是 step06 的核心改动：
//   1. ToolHandler 多了两个布尔属性
//      - supports_parallel_tool_calls: 这个工具是否可以与其它“并发安全”工具
//        同时执行（典型：read-only 类工具）；
//      - requires_dispatch_lock:        这个工具是否需要走 registry 的读写锁
//        （sub-agent 之类的工具需要 false，避免自己等自己）。
//   2. ToolRegistry 维护一个“读写锁”——并发安全的工具拿读锁可同时进行；
//      非并发安全的工具拿写锁，独占执行。这与 Rust 中的 tokio::sync::RwLock
//      行为对齐，本文件用一个最小自实现。
//   3. dispatch_many(invocations[]) 把一批 tool calls 同时发起，按下发顺序
//      返回结果。
//
// 教学意图：真实的 coding agent 会大量做“同时读 5 个文件 / 同时跑 3 条只读
// shell”的事，串行化会极大拖慢响应。
// =============================================================================
