// =============================================================================
// step04 / sandbox.ts — 引入 spawn_sub_agent
// -----------------------------------------------------------------------------
// 在 step03 的基础上：
//   1. ToolRegistry 不再无状态，而是承载 plan_state（持久化最近一次 plan）；
//   2. ToolHandler.handle 多了一个 registry 参数 —— 这样工具内部还能再次
//      调用其它工具（最关键的是 spawn_sub_agent 需要把所有原生工具借给子 agent）；
//   3. 新增 SubAgentHandler：给定一段 instruction，启动一个嵌套 agent loop，
//      跑完后把最终文本作为工具结果返回。
//
// 这一阶段开始体现：agent runtime 必须支持“委派”，而不是只能单线程线性执行。
// =============================================================================
